...
const verifyToken = (socket, next) => {
    const token = socket.handshake.headers.cookie
        ? socket.handshake.headers.cookie.split('; ').find(c => c.startsWith('userAccessToken=') || c.startsWith('adminAccessToken=')).split('=')[1]
        : null;
    ...
    const role = decoded.adminRole ? 'Admin' : decoded.userRole ? 'User' : 'Unknown';
    socket.decoded = {
        ...decoded,
        role: role
    };
...
const setupSocketIo = (server) => {
    ...
    io.on('connection', async (socket) => {
        ...
        if (socket.decoded.role === 'User') {
            ...
            image = user.image && user.image.length > 0 ? user.image[0].imageUrl : 'https://bootdey.com/img/Content/avatar/avatar5.png';
            userSockets[socket.decoded.id] = socket.id;
            const mainAdmin = await Admin.findOne({});
            socket.emit('userInfo', { userId: socket.decoded.id, userName: name, userImage: image, adminId: mainAdmin._id });
            ...
            image = admin.image && admin.image.length > 0 ? admin.image[0].imageUrl : 'https://bootdey.com/img/Content/avatar/avatar3.png';
            adminSockets[socket.decoded.id] = socket.id;
            socket.emit('adminInfo', { adminId: socket.decoded.id, adminName: name, adminImage: image });
            ...
            const userList = users.map(user => ({
                ...
                userImage: user.image && user.image.length > 0 ? user.image[0].imageUrl : 'https://bootdey.com/img/Content/avatar/avatar5.png'
            }));
            socket.emit('allUsers', userList);
        }
      
        socket.on('privateMessage', async ({ message, senderId, recipientId, senderImage, image }, callback) => {
            ...
            const recipientSocketId = userSockets[recipientId] || adminSockets[recipientId];
            if (recipientSocketId) {
                socket.to(recipientSocketId).emit('receiveMessage', { sender: name, message, senderId, senderImage, unreadCount: 1, image });
            }
            callback({ status: 'ok' });
        });
        ...
        socket.on('typing', ({ isTyping, recipientId }) => {
            const recipientSocketId = userSockets[recipientId] || adminSockets[recipientId];
            if (recipientSocketId) {
                socket.to(recipientSocketId).emit('typing', { isTyping, senderId: socket.decoded.id });
            }
        });
